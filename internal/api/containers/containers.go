...
func (h *Handler) Inspect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if goerrors.Is(err, adapter.ErrContainerNotFound) {
			httputils.WriteError(w, http.StatusNotFound, notFoundMessage(name))
			return
		}
...
// notFoundMessage renders the Docker Engine API's body for a missing container.
func notFoundMessage(name string) string {
	return "No such container: " + name
}
...
// that stops or removes a name before reusing it treats 404 as "nothing to do"
// and 500 as a hard failure, so a blanket 500 makes such clients give up and can
// leave their own state half-built.
